...
export class SerpentineLayout extends go.Layout {
    ...
    private _spacing: go.Size = new go.Size(30, 30);  // how thick each ring should be
    private _wrap: number = Infinity;
    ...
    */
    public doLayout(coll) {
        ...
        var wrap = this.wrap;
        if (diagram !== null && isNaN(this._wrap)) {
            if (this.group === null) {  // for a top-level layout, use the Diagram.viewportBounds
                var pad = diagram.padding as go.Margin;
                this._wrap = Math.max(spacing.width * 2, diagram.viewportBounds.width - 24 - pad.left - pad.right);
            } else {
                this._wrap = 1000; // provide a better default value?
            }
        }
        ...
        while (node !== null) {
            ...
            if (increasing) {
                ...
                if (x + spacing.width + nb.width > wrap) {
                    y += rowh + this._spacing.height;
                    x = this._wrap - spacing.width;
                    rowh = 0;
                    increasing = false;
                    if (nextlink !== null) {
